...
const MODULES = [
  { id: 'experiments', zh: '实验进程', en: 'Experiments', zhDesc: '实时查看 Argus 运行位置、DAG、角色交接和停止原因。', enDesc: 'Track Argus execution, DAG progress, role handoffs, and stop reasons.', icon: FlaskConical, color: 'blue' },
  { id: 'copilot', zh: 'Research Copilot', en: 'Research Copilot', zhDesc: '保留原版 Argus 对话、Prompt 优化和工具轨迹。', enDesc: 'Chat with Argus, refine prompts, and inspect tool activity.', icon: MessagesSquare, color: 'violet' },
  { id: 'literature', zh: '文献中心', en: 'Literature', zhDesc: '汇总已读论文、最近工作、检索记录和文献证据。', enDesc: 'Review papers, related work, retrieval history, and evidence.', icon: BookOpen, color: 'indigo' },
  { id: 'inbox', zh: '科研收信箱', en: 'Research Inbox', zhDesc: '从零散输入抽取知识点并形成第一版 Argus Prompt。', enDesc: 'Turn rough notes into structured knowledge and an Argus prompt.', icon: Inbox, color: 'rose' },
  { id: 'ide', zh: 'AI IDE', en: 'AI IDE', zhDesc: '连接真实服务器目录，查看代码、Git 和 Argus 活动。', enDesc: 'Browse server files, Git state, and Argus activity.', icon: Code2, color: 'emerald' },
  { id: 'paper', zh: '论文工作区', en: 'Paper Workspace', zhDesc: '自动发现 Argus 新写入的文稿、BibTeX、图表和 PDF。', enDesc: 'Discover manuscripts, BibTeX, figures, and PDFs from the workspace.', icon: FileText, color: 'amber' },
  { id: 'reviewer', zh: '模拟审稿', en: 'Reviewer', zhDesc: '区分每轮过程审稿与项目完成后的最终投稿前审稿。', enDesc: 'Separate round-level review from final pre-submission review.', icon: ShieldCheck, color: 'slate' },
  { id: 'release', zh: '成果发布', en: 'Release', zhDesc: '规划 GitHub 仓库、学术海报和项目宣传页。', enDesc: 'Plan a GitHub repository, academic poster, and project page.', icon: Megaphone, color: 'rose' },
] as const;
    ...
    <div className="overview-page">
      <section className="overview-hero">
        <div className="overview-hero__copy">
          ...
          <p>{view?.mission.objective || props.status?.continuous?.objective || props.project.objective || text('尚未设置研究目标。', 'No research objective has been set.')}</p>
        ...
        <div className="overview-hero__stats">
          ...
          <div><span>{text('研究阶段', 'Research stage')}</span><strong>{view?.stage.label || '—'}</strong><small>{view?.mission.status || 'idle'}</small></div>
