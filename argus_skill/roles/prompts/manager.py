...
def build_vertical_decision_prompt(
    ...
    *,
    ...
    return (
        "You are MANAGER. Choose a capability VERTICAL and execution WORKFLOW. "
        "A vertical is a stable reusable staged capability, not a Planner DAG.\n\n"
        "Inspect evidence with at most 3 targeted operations, each <80 lines and "
        ...
        "Choose WORKFLOW independently: `direct` for one coherent package; `staged` "
        "for dependent phases, alternatives, a broad survey plus synthesis, or "
        "research plus implementation/validation. A limited single increment is "
        "direct. For built-in "
        "`research` vertical, an exploratory task with no explicit target venue is "
        "`direct`; `research`/`staged` is the publication pipeline. Non-scholarly "
        "public-source investigations use `research`/`direct` when no exact domain fits.\n\n"
        "## Task\n"
        f"{(task or '').strip()}\n\n"
