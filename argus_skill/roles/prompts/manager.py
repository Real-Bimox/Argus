...
def build_vertical_decision_prompt(
    ...
    *,
    ...
    return (
        "You are the MANAGER. Choose the capability VERTICAL and independent "
        "execution WORKFLOW for the task. A vertical is a stable reusable staged "
        "capability, not this mission's Planner DAG.\n\n"
        "Inspect routing evidence with at most 3 targeted operations, each <80 "
        "lines and <16k characters total. Never repeat/broaden searches. Read-only; "
        ...
        "then another existing project domain. Match the requested ACTION, not nouns "
        "inside quoted filenames, logs, or commit titles; repository maintenance is "
        ...
        "`domain=chemistry`.\n"
        "   Read-only repo audit/report: `software`/`direct`, not "
        "`argus_maintenance`; no meta-review unless explicitly requested.\n"
        "2. Reuse a matching `status=candidate` domain. If its stages are materially "
        "underfit, keep its name and return the complete improved STAGES list.\n"
        "3. Choose `new` only when no listed capability fits. Define a reusable slug "
        f"and {_MIN_DOMAIN_STAGES}-{_MAX_DOMAIN_STAGES} action stages grounded in the repo. "
        ...
        "Choose WORKFLOW independently: `direct` for one coherent Engineer work "
        "package with focused validation; `staged` for dependent phases, multiple "
        "evidence tracks/alternatives, a broad survey plus synthesis, or research "
        "plus implementation/validation. One final report can still be staged. An "
        "explicitly limited single stage/increment is direct. For the built-in "
        "`research` vertical, an exploratory task with no explicit target venue is "
        "always `direct`; `research`/`staged` is the publication pipeline. Company "
        "due diligence and other non-scholarly public-source investigations are "
        "`research`/`direct` when no more exact domain fits, never "
        "`research`/`staged`.\n\n"
        "## Task\n"
        f"{(task or '').strip()}\n\n"
        f"Research-target verticals: {target_verticals}. For them, set the requested "
        "success bar: exploratory for a bounded investigation/known result/finite "
        "verification; publishable only when original publication-significance work "
        "is requested; doctoral only when explicitly thesis/doctoral-level. Rigor, "
        ...
        "Return the named lines below; optional prose is ignored. EXECUTION_TASK must "
        "stand alone and preserve every requested action, deliverable, order, and stop "
        "condition. Do not replace the goal with cleanup, docs, hashes, provenance, or "
        "extra validation unless requested or necessary. A requested reversible "
        "download/build/conversion/run authorizes a real attempt before claiming an "
        "external blocker. Do not delete/overwrite existing files merely because new "
